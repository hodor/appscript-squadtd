  ...
  public loadDataAtRow(row: number) {
    ...
    if (baseUnitName && !baseUnit)
      throw Utilities.formatString(
        "Unit %s on row %d has the undefined base unit %s. Try moving the %s above the unit %s",
        name,
        row,
        baseUnitName,
        baseUnitName,
    ...
    let rowUnit: PlayerUnit = new PlayerUnit(
      name,
      cost,
      baseUnit,
      null,
    ...
    Logger.log(
      Utilities.formatString(
        "Creating unit[%s], which has a base type %s. Armor[%s], Weapon[%s], DPS[%f]",
        rowUnit.name,
        baseUnitName,
        rowUnit.armorType,
        rowUnit.attackType,
