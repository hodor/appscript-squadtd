  ...
  export class Calculator {
    ...
    public static isUnitTypeValid(unit: string): boolean {
      for (const key in UnitType) {
        ...
        if (value.toUpperCase() === unit.toUpperCase()) return true;
      }
      return false;
    }
    ...
    public static isDamageTypeValid(damage: string): boolean {
      for (const key in DamageType) {
        ...
        if (value.toUpperCase() === damage.toUpperCase()) return true;
      }
      return false;
    }
    ...
    public static baseDamage(damage: string, unit: string): number {
      ...
      if (damage == DamageType.chaos || unit == UnitType.biological) return 1;
      ...
      throw Utilities.formatString(
        "Could not find any relationship between damage type %s and unit type %s",
        damage,
        unit
      );
    }
